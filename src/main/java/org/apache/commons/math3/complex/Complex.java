...
public class Complex implements FieldElement<Complex>, Serializable {
    ...
    public Complex reciprocal() {
        if (this.isNaN) {
            ...
            if (FastMath.abs(this.real) < FastMath.abs(this.imaginary)) {
                q = this.real / this.imaginary;
                scale = 1.0 / (this.real * q + this.imaginary);
                return this.createComplex(scale * q, -scale);
            } else {
                q = this.imaginary / this.real;
                scale = 1.0 / (this.imaginary * q + this.real);
    ...
    public Complex multiply(Complex factor) throws NullArgumentException {
        ...
        if (!this.isNaN && !factor.isNaN) {
            return !Double.isInfinite(this.real) && !Double.isInfinite(this.imaginary) && !Double.isInfinite(factor.real) && !Double.isInfinite(factor.imaginary) ? this.createComplex(this.real * factor.imaginary + this.imaginary * factor.real,this.real * factor.real - this.imaginary * factor.imaginary) : INF;
        } else {
            return NaN;
        }
    }
    ...
    public Complex sqrt() {
        if (this.isNaN) {
            ...
            return this.real >= 0.0 ? this.createComplex(t, this.imaginary / (2.0 * t)) : this.createComplex(FastMath.abs(this.imaginary) / (2.0 * t), FastMath.copySign(1.0, this.imaginary) * t);
        }
    }
    ...
    public List<Complex> nthRoot(int n) throws NotPositiveException {
        if (n <= 0) {
            ...
            if (this.isNaN) {
                ...
                double slice = 6.283185307179586 / (double)n;
